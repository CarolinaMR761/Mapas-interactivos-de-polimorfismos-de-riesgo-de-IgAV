...
# Países de interés
paises_latam <- c("Mexico", "Colombia", "Peru",
...
polymorphism_data <- data.frame(
  name = paises_latam,
  frequency = c(100, 47.4, 47.4, 47.4, 47.4, 47.4, 47.4, 47.4, 47.4, 47.4,
                47.4, 47.4, 47.4)
)
...
leaflet(world_latam) %>%
  ...
  addLegend(
    ...
    pal = pal_turbo,
    values = ~frequency,
    title = "Frecuencia del alelo ID (%)",
    opacity = 1
  ) %>%
  addControl(
    "<b>Frecuencia del alelo de riesgo (ID)<br>Polimorfismo I/D del gen ACE</b>",
    position = "topright"
  ) %>%
  addControl(
    html = "<small>Fuente de datos:<br/>
  Molecular and Cellular Biochemistry (SPRINGER NATURE) – <a href='https://doi.org/10.1007/s11010-023-04923-9' target='_blank'>ACE I/D Sudámerica</a><br/>
  Human Biology (AAAG) – <a href='https://digitalcommons.wayne.edu/humbiol/vol75/iss6/6' target='_blank'>ACE I/D México</a><br/>",
    position = "bottomleft"
  )
